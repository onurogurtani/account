...
namespace TurkcellDigitalSchool.Account.Business.Handlers.GreetingMessages.Commands
{
    ...
    public class UpdateGreetingMessageCommand : IRequest<DataResult<GreetingMessageDto>>
    {
        public GreetingMessage Entity { get; set; }

        [MessageClassAttr("Karşılama Mesajı Oluþtur")]
        public class UpdateGreetingMessageCommandHandler : IRequestHandler<UpdateGreetingMessageCommand, DataResult<GreetingMessageDto>>
        {
            private readonly IGreetingMessageRepository _greetingMessageRepository;
            private readonly IFileRepository _fileRepository;
            private readonly IMapper _mapper;
            public UpdateGreetingMessageCommandHandler(IGreetingMessageRepository greetingMessageRepository, IMapper mapper, IFileRepository fileRepository)
            {
            ...
            public async Task<DataResult<GreetingMessageDto>> Handle(UpdateGreetingMessageCommand request, CancellationToken cancellationToken)
            {
                if (request.Entity.HasDateRange)
                {
                    var isDateConflict = _greetingMessageRepository.Query()
                                          .Where(w => w.Id != request.Entity.Id)
                ...
                _greetingMessageRepository.Update(request.Entity);
