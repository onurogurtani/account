...
const HandleContractFormButton = ({ form, initialValues, contractTypes, contractKinds }) => {
  ...
  const onFinish = useCallback(async (contractTypes) => {
    ...
    try {
      const values = await form.validateFields();
      const startDate = values?.validStartDate ? dayjs(values?.validStartDate)?.utc().format('YYYY-MM-DD') : undefined;
      const startHour = values?.validStartDate ? dayjs(values?.validStartDate)?.utc().format('HH:mm:ss') : undefined;
      ...
      const res = await dispatch(getContractTypes(typeData));
