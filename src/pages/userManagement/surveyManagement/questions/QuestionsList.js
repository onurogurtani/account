...
const QuestionsList = () => {
            ...
            render: (_, record) => {
                const menu = (
                    <Menu>
                        {menuList.map(
                            (item) =>
                                !(item.key === '4' && record.isActive === false) &&
                                !(item.key === '5' && record.isActive === true) && (
                ...
                return (
                    <Dropdown type="primary" overlay={menu}>
                        <Button className="ant-dropdown-link userName">
                            İşlemler
                            <DownOutlined />
                        </Button>
                    </Dropdown>
    ...
    const loadData = async () => {
        const res = await dispatch(getQuestions(filterParams));
        const res2 = await dispatch(getLikertType());
    };

    const loadQuestionTypes = async () => {
        if (questionType.length === 0) {
            let types = await dispatch(getQuestionsType()).unwrapResult();
    ...
    return (
        <>
            <CustomCollapseCard className="draft-list-card" cardTitle={<Text t="Soru Listesi" />}>
                {
