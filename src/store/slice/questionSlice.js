...
export const getQuestions = createAsyncThunk('question/getQuestions', async (data) => {
  ...
  if (data) {
    ...
    urlString =
      'QuestionDetailSearch.OrderBy=insertDESC&QuestionDetailSearch.PageNumber=1&QuestionDetailSearch.PageSize=10';
  }
...
// Likert Type
export const getLikertType = createAsyncThunk('getLikertType', async () => {
...
export const questionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(addQuestions.fulfilled, (state, action) => {
      state.questionId = action?.payload?.data?.id;
    });
